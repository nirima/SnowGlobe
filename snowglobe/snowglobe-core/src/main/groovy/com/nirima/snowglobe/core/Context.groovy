...
abstract class Context<T> {
    ...
    def propertyMissing(String name) {
        if( name.equals("env") ) {
            return getSnowGlobeContext().parameters;
        }
        throw new MissingPropertyException(name, getClass());
    }

    abstract SnowGlobeContext getSnowGlobeContext();
...
public class SnowGlobeContext extends Context<SnowGlobe> {
    ...
    SnowGlobeContext(SnowGlobe proxy, SGParameters parameters) {
        super(proxy);
        this.parameters = parameters;
    }

    SnowGlobeContext(SnowGlobe proxy) {
        this(proxy, new SGParameters());
    }

    SnowGlobeContext getSnowGlobeContext() {
        return this;
    }
    ...
    def invokeMethod(String name, Object args) {
        try {
            ...
            throw new RuntimeException("Cannot invoke ${name}", ex);
        }
    }
    ...
    private void processModule(ModuleContext context) {
        ...
        try {
            ...
            throw new RuntimeException("Error processing module ${context}",ex);
        }
...
//    }
    ...
    public Script load(String relativePath) {
        Script s1 = getProxy().load(relativePath);
        s1.setDelegate(this);
        return s1;
    }

    public Script loadModule(String uri) {
        Script s1 = getProxy().loadModule(uri);
        s1.setDelegate(this);
        return s1;
    }

    public void evaluateScript(DelegatingScript script1) {
        // def r = script1.run();

        //    r.delegate = this;
        //     r.resolveStrategy = Closure.DELEGATE_FIRST;
        script1.setDelegate(this);
        script1();
...
public class ProviderContext extends Context<Provider> {
    ...
    @Override
    SnowGlobeContext getSnowGlobeContext() {
        return parent.getSnowGlobeContext();
    }
}
...
public class ResourceContext extends Context<Resource> {
    ...
    @Override
    SnowGlobeContext getSnowGlobeContext() {
        return moduleContext.getSnowGlobeContext();
    }
}